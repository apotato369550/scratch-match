...
export async function POST(req: Request) {
  ...
  if (isShowcaseMode()) {
    return NextResponse.json({ job_id: 99, chunks: 10, showcase: true });
  }
  const openCount = (
    db.prepare("SELECT COUNT(*) AS n FROM jobs WHERE employer_id = ? AND status = 'open'").get(user.id) as { n: number }
  ).n;
  if (openCount >= MAX_OPEN) {
    return NextResponse.json({ error: `Limit reached: ${MAX_OPEN} open jobs per employer.` }, { status: 409 });
  }
  try {
